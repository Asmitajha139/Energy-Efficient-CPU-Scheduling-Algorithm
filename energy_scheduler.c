...
void add_task(Scheduler *sched, int id, int exec_time, int arrival_time, int priority) {
    ...
    while (1) {
        ...
        for (int i = 0; i < sched->task_count; i++) {
            if (!sched->tasks[i].completed && sched->tasks[i].arrival_time <= sched->current_time) {
                if (sched->tasks[i].priority > max_priority) {
                    max_priority = sched->tasks[i].priority;
                }
            }
        }
        for (int i = 0; i < sched->task_count; i++) {
            if (!sched->tasks[i].completed && 
                sched->tasks[i].arrival_time <= sched->current_time && 
                sched->tasks[i].priority == max_priority) {
                ...
                printf("Time %dms: Batched Task %d (Priority %d), exec time %dms\n", 
                       sched->current_time + batch_time, t->task_id, t->priority, t->exec_time);
            }
        }
